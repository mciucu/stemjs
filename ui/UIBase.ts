...
// The onX option each addXListener implies, called with the adder's arguments and then the element.
// An adder that declares its callback open - addChangeListener's `Function` - names no option: the option
// would type nothing, and being on every element it would defeat the narrower ones written by hand
export type EventOptions<Element> = {
    [K in Extract<keyof Element, `add${string}Listener`> as
        Element[K] extends (callback: (...args: infer Args) => any, ...rest: any[]) => any
            ? number extends Args["length"]
                ? never
                : (K extends `add${infer Event}Listener` ? `on${Event}` : never)
            : never]?:
                Element[K] extends (callback: (...args: infer Args) => any, ...rest: any[]) => any
                    ? (...args: [...Args, Element]) => any
                    : never;
};
...
> extends BaseUIElement<NodeType> {
    ...
    // Written by addListenersFromOptions, one per option it has wired; nothing else reads them
    [handler: `on${string}AutoHandler`]: ((...args: any[]) => void) | undefined;
    ...
    addListenersFromOptions() {
        for (const key in this.options) {
            if (typeof key === "string" && key.startsWith("on") && key.length > 2) {
                ...
                const addListenerMethodName: `add${string}Listener` = `add${eventType}Listener`;
                const handlerMethodName: `on${string}AutoHandler` = `on${eventType}AutoHandler`;
                ...
                // Left standing: an `add${string}Listener` index signature would swallow every adder name in EventOptions
                ...
                if (haveListenerMethod || nodeEvent) {
                    this[handlerMethodName] = (...args: any[]) => {
    ...
    addClickListener(callback: (event: MouseEvent) => unknown): RemoveHandle {
    ...
    // Named so EventOptions derives them; the nodeEventsMap fallback serves the same two events
    addMouseEnterListener(callback: (event: MouseEvent) => void): RemoveHandle {
        return this.addNodeListener("mouseenter", callback);
    }

    addMouseLeaveListener(callback: (event: MouseEvent) => void): RemoveHandle {
        return this.addNodeListener("mouseleave", callback);
    }
